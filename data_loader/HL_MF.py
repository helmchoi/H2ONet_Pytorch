...
class HL_MF(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            bbox = np.array([500.0, 130.0, 450.0, 450.0]).astype(np.float32) # get_bbox(joints_coord_img[:, :2], np.ones_like(joints_coord_img[:, 0]), expansion_factor=1.5)
    ...
    def __getitem__(self, idx):
        ...
        frame_idx_0 = int(filename.split(".")[0]) #.split("_")[0])
        ...
        input = {}
        for img_idx, (img, data, img2bb_trans, bb2img_trans, rot, img_shape) in enumerate(zip(img_list, data_list, img2bb_trans_list, bb2img_trans_list, rot_list, img_shape_list)):
            assert self.data_split != "train"
